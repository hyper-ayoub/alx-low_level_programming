...
/**
 * my_chain - test if the current char in the buffer is a chain delimiter
 * @myinfo: the parameter struct
 * @buf: the char buffer
 * @po: address of the current position in buf
 *
...
/**
 * chk_chain - checks if we should continue chaining based on the last status
 * @myinfo: the parameter struct
 * @buf: the char buffer
 * @po: address of the current position in buf
 * @_i: starting position in buf
 * @_len: length of buf
 *
 * Return: Void
 */
...
/**
 * replace_myalias - replaces an alias in the tokenized string
 * @myinfo: the parameter struct
 *
 * Return: 1 if replaced, 0 otherwise
 */
int replace_myalias(info_t *myinfo)
{
	int i;
	list_t *node1;
	char *po;
	...
	for (i = 0; i < 10; i++)
	{
		node1 = node1_starts(myinfo->alias, myinfo->argv[0], '=');
		if (!node1)
			return (0);
		free(myinfo->argv[0]);
		po = strchr(node1->str, '=');
...
/**
 * replace_myvars - replaces variables in the tokenized string
 * @myinfo: the parameter struct
 *
 * Return: 1 if replaced, 0 otherwise
 */
